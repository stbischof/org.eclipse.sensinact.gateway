...
# .mvn/maven.config at the repository root. Read it from there so the docs
# can never drift from the build; -SNAPSHOT is dropped for display.
...
release = _revision.group(1).removesuffix("-SNAPSHOT")
version = release
