...
import org.eclipse.sensinact.gateway.tools.exporter.feature.FeatureExporter;
...
public class FeatureExporterTest {
    ...
    private Path genWsPath;

    @Test
    public void testBase(@TempDir Path tmp) throws Exception {
        ...
        Assertions.assertThat(jsonText).containsEntry("id", "TheGroupId:TheSymbolicName:0.0.0-INITIAL")
                .containsEntry("name", "TheName").containsEntry("description", "TheDescription")
                .containsEntry("vendor", "TheVendor").containsEntry("license", "TheLicense")
                .containsEntry("categories", Lists.of("TheCategory")).containsEntry("docurl", "TheDocUrl")
    ...
    private Map<Object, Object> process(Path tmpDir, String bndrun) throws IOException, Exception {
        genWsPath = tmpDir.resolve( WS_PATH);
        final File wsRoot = genWsPath.toFile();
        IO.delete(wsRoot);
        ...
        try (Workspace ws = new Workspace(wsRoot)) {
            ...
            try (Run subsys = Run.createRun(ws, fsubs)) {
                ...
                try (FileResource featureResource = (FileResource) featureMap.getValue()) {
                    return new JSONCodec().dec().from(featureResource.getFile()).get(Map.class);
                }
            }
        }
    }
