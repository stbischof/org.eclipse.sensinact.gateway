...
when the version is not a `-SNAPSHOT`.
...
exactly one place: the `-Drevision=` line in
[`.mvn/maven.config`](.mvn/maven.config). The poms only reference
`${revision}` - never add a second copy of the version anywhere else. The
documentation reads its version from the same line.

A release is the creation of a git tag whose name equals that version,
without any prefix: `0.0.2`, not `v0.0.2`.

1. Open a pull request that sets `-Drevision=0.0.2` (no `-SNAPSHOT`) in
...
5. Open a pull request that sets the next development version, for example
   `-Drevision=0.0.3-SNAPSHOT`, and merge it. SNAPSHOT deploys resume.
...
`.mvn/maven.config`, and refuses `-SNAPSHOT` versions, so a tag alone can
